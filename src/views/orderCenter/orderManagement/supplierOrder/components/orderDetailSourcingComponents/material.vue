  ...
  <div class="order-detail-material">
          ...
          <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
              <a-form-model-item label="商品名称" prop="productCname">
                <a-input placeholder="请输入平台商品编码" v-model="queryParam.productCname" />
              </a-form-model-item>
            </a-col>
            <a-col :md="8" :sm="24">
            ...
            <a-col :md="8" :sm="24">
              <a-form-model-item label="采购员" prop="buyerUserName">
                <a-input placeholder="请输入品牌/产地" v-model="queryParam.buyerUserName" />
              </a-form-model-item>
            </a-col>
          </a-row>
        </a-form-model>
    ...
    >
      ...
      <!-- <template slot="options" slot-scope="item">
        ...
        <a-popconfirm title="确定删除该物料明细吗?" ok-text="删除" cancel-text="取消" @confirm="delItem(item)">
          <a-button class="options" type="link" v-show="item.id != id" :disabled="id != null && item.id != id">
            删除
          </a-button>
        </a-popconfirm>
  ...
  methods: {
    async init(params) {
      const formData = {
        ...
        startDate: this.queryParam.date ? moment(this.queryParam.date[0]).format('yyyy-MM-DD') : null,
        endDate: this.queryParam.date ? moment(this.queryParam.date[1]).format('yyyy-MM-DD') : null
      }
      this.cache = null
      this.id = null
      return api.getOrderMaterialList(formData)
    },
    handelChange(item, key) {
      if (['productPriceSale', 'productItemNum'].includes(key)) {
        item.productItemAmount = (Number(item.productPriceSale) * Number(item.productItemNum)).toFixed(2)
        item.priceWith = (Number(item.productPriceSale) / (1 + Number(item.taxRate) / 100)).toFixed(2)
        item.withoutTaxTotalPrice = (Number(item.priceWith) * Number(item.productItemNum)).toFixed(2)
      }
    },
    search() {
