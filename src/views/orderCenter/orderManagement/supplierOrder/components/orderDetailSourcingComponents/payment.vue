    ...
    <div class="l">
      ...
      <div class="table" v-else>
        <a-table
          ...
          :columns="columns"
        >
          <template v-for="item in inputKeys" :slot="item.key" slot-scope="props, i, index">
            <d-slot-fileds
              :disabled="disableds || isEnableSelect(index, item.key)"
              :type="item.type"
              :min="item.min"
              :max="item.max"
          ...
          <template v-if="!disableds" slot="opera" slot-scope="props, i, index">
            <a-button
              v-if="index !== form.manualOrderPaymentList[0].phasePayment.length - 1"
            ...
            <a-button
              v-if="index !== form.manualOrderPaymentList[0].phasePayment.length - 1 && index !== 0"
              style="min-width: 0; padding: 0 4px"
              type="link"
...
export default {
  ...
  methods: {
    ...
    handleAddLine(index) {
      this.form.manualOrderPaymentList[0].phasePayment.splice(index, 0, {
        type: this.form.manualOrderPaymentList[0].phasePayment[index].type,
        day: '',
        percent: ''
      })
    },
