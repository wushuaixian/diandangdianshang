 ...
 * @LastEditTime: 2021-07-24 16:25:31
  ...
  <div>
    <a-modal title="导出任务列表" :width="800" v-model="visible" @ok="handleOk" @cancel="handleCancel">
      ...
      >
        ...
        <template slot="action" slot-scope="action, item" v-if="item.filePath">
...
import { soExportTaskScheduleListPage } from '@/api/orderCenter'
const columns = [
  {
    title: '任务编号',
    dataIndex: 'id',
  ...
  },
  {
    title: '操作类型',
    width: 200,
    dataIndex: 'taskActionTypeStr',
    key: 'taskActionTypeStr'
  },
  {
    title: '任务状态',
...
export default {
  ...
  methods: {
    ...
    async getList(params) {
      ...
      const result = await soExportTaskScheduleListPage({
        filters: { ...queryParam, taskType: '1' },
        page: currentPage,
        limit: itemsPerPage
      })
      const res = {
