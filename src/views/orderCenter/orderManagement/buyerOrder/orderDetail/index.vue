<template>
  <section class="add-order">
    <div class="item-box"><Header :disableds="disabled" :form="form" /></div>
    <div class="item-box">
    ...
    <div class="item-box">
      ...
      <Material ref="materialList" :disableds="disabled" :form="form" />
    </div>
    <div class="item-box">
      <header-title class="title"><span>付款方式</span></header-title>
      <Payment ref="paymentform" :disableds="disabled" :form="form" />
    </div>
    <div class="item-box">
      <header-title class="title"><span>收货地址</span></header-title>
      <Address :form="form" :disableds="disabled" type="DEFAULT" />
    </div>
    <div class="item-box">
      <header-title class="title"><span>发票信息</span></header-title>
      <Invoice :disableds="disabled" :form="form" />
    </div>
    <div class="item-box">
      <header-title class="title"><span>发票地址信息</span></header-title>
      <Address :disableds="disabled" :form="form" type="INVOICE" />
    </div>
    <div class="foot-btn">
      ...
      <a-button
        class="btns"
        v-if="[1011, 1041, 1063].includes(Number(form.confirmStatus)) && !disabled"
        type="primary"
        @click="Save(1011)"
        :loading="saveLoading"
      >
      ...
      <!-- v-if="[1011, 1041, 1063, 1030].includes(Number(form.confirmStatus))" -->
      <a-button
        class="btns"
        v-if="[1011, 1041, 1063].includes(Number(form.confirmStatus)) && !disabled"
        type="primary"
...
<script>
import { Payment, Address, Invoice } from '../addOrder/components/index'
import { Header, Basic, Material } from './components'
...
export default {
  ...
  created() {
    ...
    if (this.form.purchaseOrderSource !== 1) {
      this.BindData()
    }
  },
  data() {
    return {
      disabled: this.disableds || this.$route.query?.type === 'read',
      form: {
        remark: '',
  ...
  methods: {
    BindData() {
      api.getOrderDetail(this.$route.query.orderCode).then((res) => {
        if (res.code === '0') {
          if ([1050, 1062, 9000].includes(Number(res.data.so.orderStatusCode))) {
            this.disabled = true
          }
          const form = {
            ...
            isQuality: res.data.so.isQuality, // 是否优质优价(0.否 1.是)
            isJinGauging: res.data.so.isJinGauging, // 是否检斤检尺(0.否 1.是)
            qualityRetentionMoney: '', // 质保金
            qualityRetentionPeriod: '', // 质保期
            jinGaugingStartDate: res.data.so.jinGaugingStartDate, // 检斤开始时间(yyyy-mm-dd hh:mm:ss)
            jinGaugingEndDate: res.data.so.jinGaugingEndDate, // 检斤结束时间(yyyy-mm-dd hh:mm:ss)
            customerRemark: '', // 订单备注(用户)
            ...
            manualOrderPaymentList: [
              {
                ...
                paymentType:
                  res.data.so?.soPaymentDTOList?.length > 0 ? res.data.so?.soPaymentDTOList[0]?.paymentType : 1, // 付款方式(1:账期 2:分阶段)
                payType: res.data.so?.soPaymentDTOList?.length > 0 ? res.data.so?.soPaymentDTOList[0]?.payType : 1, // 支付方式(1:电汇 2:承兑汇票 3:两者都是)
                paymentDaysType:
                  res.data.so?.soPaymentDTOList?.length > 0 ? res.data.so?.soPaymentDTOList[0]?.paymentDaysType : 1, // 结算账期类型(1:每年 2:每季度 3:每月 4:每周 5:每日 6:自定义)
            ...
            // 发票信息
            manualOrderInvoiceVo: {
              invoiceTitle: res.data.so.soInvoiceDTO?.invoiceTitleContent, // 发票抬头
              mobileNo: res.data.so.soInvoiceDTO?.registerPhone,
    ...
    Save(confirmStatus) {
      this.validate(() => {
        ...
        const formData = {
          ...
          soPaymentDTOList: [
            {
              id: this.form.manualOrderPaymentList[0].id,
