<template>
  <div>
    <a-modal okText="下一步" v-model="visible" title="基本信息" @ok="anextStep">
      <a-row>
        <a-col :span="24">
          <h3 class="crude">
            你已缴纳的平台保证金总额
    ...
    <a-modal okText="立即前往" v-model="visibles" title="缴纳保证金" @ok="nextSteps">
      <a-row>已成功为您创建应付单,请前往<span @click="go" id="go">应付单管理</span>查看</a-row>
    </a-modal>
    <a-modal okText="提交" v-model="avisible" title="缴纳保证金" @ok="anextStep">
...
export default {
  ...
  methods: {
    anextStep() {
      if (this.form.feeAmt === '' || Number(this.form.feeAmt) === 0) return this.$message.info('请输入正确金额')
      this.nextStep()
      this.visibles = true
      this.visible = false
      this.avisible = false
    ...
    nextStep() {
      if (this.form.feeAmt && this.form.feeAmt > 0) {
