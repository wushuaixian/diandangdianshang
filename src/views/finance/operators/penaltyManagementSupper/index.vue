  ...
  <div class="penaltyManagement">
    ...
    <a-card>
      ...
      <div class="buttonAraay">
        <a-button type="primary" @click="CreateTicket">创建罚款单</a-button>
      ...
      <s-table rowKey="id" name="homePageTable" ref="table" :scroll="{ x: true }" :columns="columns" :data="init">
        ...
        <template slot-scope="text" slot="action">
          <span class="cursor" @click="relaunch(text)" v-show="text.billStatus === 2">重新发起</span>
        </template>
    ...
    <CreateTicket
      ...
      "
    />
    <FineDetails :visible="fineDetailsVisible" @close="fineDetailsVisible = false" :params="fineDetailParams" />
    <SystemctlTips :visible="systemctlTipsVisible" @close="systemctlTipsVisible = false" :code="exportCode" />
    <ViewTasks :visible="viewTasksVisible" @close="viewTasksVisible = false" taskType="platformForfeitInfoExport" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    exportFile() {
      ...
      exportInvoice({
        templateCode: 'platformForfeitInfoExport',
        jsonTxt,
        exportType: 'exportForfeitInfoList',
        fileName: '罚款信息导出列表'
      }).then((res) => {
        if (res.code === '0') {
          exportId({ taskType: 'platformForfeitInfoExport' }).then((res) => {
            if (res.code === '0') {
