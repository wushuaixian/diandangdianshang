...
// import { queryUserManagementInfo } from '@/api/userManagement'
...
export default {
  ...
  methods: {
    ...
    async load() {
      // const { merchantId: orgId } = this.info
      this.list[this.brandIndex].infiniteScrollDisabled = true
      if (this.brandIndex === 0) {
        //  查询审批人列表
        const { data: listObj } = await listUserDepartment({
