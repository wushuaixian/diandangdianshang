<template>
  <div class="cp-audit-add">
    ...
    <template v-if="showCheckedCount">
      ...
      <div class="add-item">
        <div v-for="(item, selectedIndex) in innerSelected" :key="selectedIndex">
          <a-tag closable @close="(e) => handleClose(item, e)">
            {{ item.actorName }}
          </a-tag>
          <span v-if="!item.hasDirector" style="color: red">暂无部门主管</span>
        </div>
      </div>
    </template>
    <a-modal
...
import {
  ...
  listUserDepartment
} from '@/api/configgurationCenter/department'
// import { queryDepartmentList } from '@/api/organization'
...
export default {
  ...
  methods: {
    ...
    async load() {
      const { merchantId: entityId, platformId } = this.info
      this.list[this.brandIndex].infiniteScrollDisabled = true
      if (this.brandIndex === 0) {
        //  查询审批人列表
        const { data } = await listUserDepartment({
          departmentType: '1',
          entityId,
          platformId
        })
        const list = data
          .filter((x) => x.parentCode !== '1')
          .map((x) => {
            return {
              ...x,
              id: x.code + '',
