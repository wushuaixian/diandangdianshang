<template>
  <div>
    ...
    <FinanceState v-if="data" :info="data" />
    <div class="finContent">
      <a-form :form="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 12 }">
        ...
        <a-form-item label="支行联行号">
          <span>{{ data.bankBranchNumer }}</span>
        </a-form-item>
      </a-form>
    </div>
      ...
      <div class="finContent">
        <a-form :form="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 12 }">
          <a-form-item label="证件类型">
            <span v-if="data.certificateType == 1">身份证</span>
            <span v-else-if="data.certificateType == 2">护照</span>
            <span v-else-if="data.certificateType == 4">港澳居民往来内地通行证</span>
            <span v-else-if="data.certificateType == 5">台胞证</span>
          </a-form-item>
          <a-form-item label="法定代表人身份证">
            <div class="IDcard">
              <ImagePreview :value="data.certificateFrontUrl"></ImagePreview>
            </div>
            <div class="IDcard">
              <ImagePreview :value="data.certificateReverseUrl"></ImagePreview>
            </div>
          </a-form-item>
          ...
          <a-form-item label="证件有效期">
            <span>{{ data.certificateValid }}</span>
          </a-form-item>
        </a-form>
      </div>
    </div>
