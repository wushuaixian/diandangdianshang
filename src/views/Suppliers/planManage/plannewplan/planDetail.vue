  ...
  <div class="new-plan">
    <a-card type="inner" :bordered="false">
          ...
          <a-row :gutter="48">
            <a-col :md="8" :sm="24" style="margin-bottom: 10px">
              <a-form-model-item label="公司名称">
                <d-select
                  :disabled="!isEdit"
                  class="max-width"
                  v-model="params.buyerMerchantId"
            ...
            <a-col :md="8" style="margin-bottom: 10px">
              <a-form-model-item label="申请组织" prop="applyDepartment">
                <a-input
                  :disabled="!isEdit"
                  :maxLength="30"
                  v-model="params.applyDepartment"
                  placeholder="请输入申请组织"
                />
              </a-form-model-item>
            </a-col>
            ...
            <a-col :md="8" style="margin-bottom: 10px">
              <a-form-model-item label="计划类型">
                <d-select :disabled="!isEdit" v-model="params.planType" :options="planTypeList"></d-select>
              </a-form-model-item>
            </a-col>
            <a-col :md="8" style="margin-bottom: 10px">
              <a-form-model-item label="申报人" prop="declarant">
                <a-input :disabled="!isEdit" :maxLength="10" v-model="params.declarant" placeholder="请输入申报人" />
              </a-form-model-item>
            </a-col>
            <a-col :md="8" style="margin-bottom: 10px">
              <a-form-model-item label="联系电话" prop="contact">
                <a-input :disabled="!isEdit" :maxLength="20" v-model="params.contact" placeholder="请输入联系电话" />
              </a-form-model-item>
            </a-col>
            <a-col :md="8" style="margin-bottom: 10px">
              <a-form-model-item label="采购申请编号" prop="buyerPlanCode">
                <a-input :disabled="!isEdit" v-model="params.buyerPlanCode" placeholder="请输入采购申请编号" />
              </a-form-model-item>
            </a-col>
            <a-col :md="8" style="margin-bottom: 10px">
              <a-form-model-item label="计划项目编码">
                <a-input
                  :disabled="!isEdit"
                  :maxLength="30"
                  v-model="params.planProjectCode"
                  placeholder="请输入计划项目编码"
                />
              </a-form-model-item>
            </a-col>
            <a-col :md="8" style="margin-bottom: 10px">
              <a-form-model-item label="计划项目名称">
                <a-input
                  :disabled="!isEdit"
                  :maxLength="50"
                  v-model="params.planProjectName"
                  placeholder="请输入计划项目名称"
                />
              </a-form-model-item>
            </a-col>
            <!-- <a-col :md="8" style="margin-bottom: 10px">
            ...
            <a-col :md="8">
              <a-form-model-item label="">
                ...
                <a-checkbox :disabled="!isEdit" v-model="params.defaultBuyerUser">默认创建人为采购员</a-checkbox>
              </a-form-model-item>
            </a-col>
          </a-row>
          <div class="table-page-search-submitButtons">
            <a-button v-if="!showHide" type="primary" @click="nextStep">下一步</a-button>
            <a-button v-else-if="params.auditStatus === 3" type="primary" @click="Submit">
              {{ !isEdit ? '编辑' : '提交' }}
            </a-button>
            <a-button v-if="params.auditStatus === 3" @click="deleteFn">删除</a-button>
      ...
      <div v-if="showHide" class="contest">
        <div class="buttons">
          <a-button :disabled="!isEdit" type="primary" @click="handleMaterial">选择物料</a-button>
          <a-button :disabled="!isEdit" type="primary" @click="handlePaginationChange(), (showImport = true)">
            模板导入
          </a-button>
        </div>
        <s-table
          ref="table"
          name="detailPlanTable"
          :pagination="{
        ...
        >
          ...
          <template v-slot:action="record" v-if="isEdit">
            <a
              class="marginleft"
      ...
      <appdflow-progress ref="progress" type-code="buyer_plan" :biz-code="$route.query.id" />
      <Import
        :disabled="!isEdit"
        ...
        @ok="search"
      />
      <MaterialModal :visible.sync="showMaterial" :merchantId="$$(dataBuyerPlanPO, 'buyerMerchantId')" @ok="handleOk" />
    </a-card>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      instancyList,
      inputKeys: [
        { key: 'planNum', type: Enum.NUMBER },
        { key: 'formerlyPrice', type: Enum.NUMBER },
  ...
  methods: {
    async getInfo() {
      const { data } = await getBuyerPlanInfoById({
    ...
    async nextStep() {
      ...
      this.dataBuyerPlanPO = this.params
      // const data = {
      //   ...params,
    ...
    async Submit() {
      ...
      const buyerPlanItemPOList = this.$refs?.table.localDataSource
      await this.handlePaginationChange()
      const res = await buyerPlanUpdate({ ...this.dataBuyerPlanPO, ...data, buyerPlanItemPOList, auditStatus: 1 })
      if (res.code === '0') {
        ...
        data: {
          listObj: result?.data?.listObj?.map((i) => {
            return {
              ...i,
              ...params,
              planProjectItemName: params.planProjectName,
              planProjectItemCode: params.planProjectCode
            }
          }),
